...
function addProperty() {
  ...
  local name=$2
  local value=$3
  echo "addProperty $path $name $value done"
...
function addPropertyLine() {
  ...
  local name=$2
  local value=$3
  echo "addPropertyLine $path $name $value"

  local entry="${name} ${value}"
  echo "addPropertyLine entry: $entry"
  local escapedEntry=$(echo "$entry" | sed 's/\//\\\//g')
  echo "addPropertyLine escapedEntry: $escapedEntry"
  # Corrected sed command to append a new line with the entry
  sed -i "\$a${escapedEntry}" "$path"
}

function configureLine() {
    echo "configureLine $path $module $envPrefix CHECK"
    local path=$1
    local module=$2
    local envPrefix=$3
...
# For Zeppelin 
echo "Configuring zeppelin-site.xml"
configure /etc/zeppelin/zeppelin-site.xml zeppelin ZEPPELIN_CONF
